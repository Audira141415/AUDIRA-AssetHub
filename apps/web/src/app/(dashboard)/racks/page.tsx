...
import { 
  Server, Box, Search, Plus, Edit2, Trash2, X, DoorOpen
} from "lucide-react"
...
export default function RacksPage() {
  ...
  const totalRacks = locations.length;
    ...
    <div className="relative min-h-[calc(100vh-80px)] overflow-hidden flex">
      <div className="flex-1 transition-all duration-500 ease-in-out p-6 md:p-8 overflow-y-auto">
        ...
        <div className="space-y-8 pb-12 mt-2">
          ...
          <HeroSection 
            title="Server Racks" 
            description="Manage server racks and their capacities" 
            imageSrc="/images/heroes/racks.png"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
            <Card className="rounded-[24px] border-neu shadow-neu-extruded bg-background">
              <CardContent className="p-5 flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-[#14B8A6]/10 flex items-center justify-center shadow-neu-inset text-[#14B8A6]">
                  <Server className="w-6 h-6" />
                </div>
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-[#14B8A6] uppercase tracking-wider mb-0.5 truncate">Total Racks</p>
                  <p className="text-2xl font-display font-bold text-foreground truncate leading-none">{totalRacks}</p>
                </div>
              </CardContent>
            </Card>
          ...
          <Card className="rounded-[32px] border-neu shadow-neu-extruded bg-background overflow-hidden">
            <div className="overflow-x-auto pb-4">
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr>
                    <th className="px-6 py-6 font-bold text-xs uppercase tracking-wider text-muted-foreground border-b border-[#A3B1C6]/20">Rack Name</th>
                ...
                <tbody className="divide-y divide-[#A3B1C6]/10">
                  {isLoading ? (
                    <tr><td colSpan={4} className="px-6 py-12 text-center text-muted-foreground font-bold">Loading racks...</td></tr>
                  ) : filtered.length === 0 ? (
                    <tr><td colSpan={4} className="px-6 py-12 text-center text-muted-foreground">No racks found.</td></tr>
                  ) : (
                    filtered.map(l => (
                        ...
                        <td className="px-6 py-5">
                          {l.parentLoc ? (
                            <div className="flex items-center gap-2 text-muted-foreground font-medium">
                              ...
                              <span>{rooms.find(s => s.id === l.parentLoc)?.name || "Unknown Room"}</span>
                            </div>
                          ) : (
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-xl h-full bg-background shadow-[-10px_0_30px_rgba(0,0,0,0.1)] border-l border-white/50 flex flex-col animate-in slide-in-from-right duration-300">
            <div className="px-8 py-6 flex justify-between items-center border-b border-[#A3B1C6]/20">
              <div>
                <h2 className="text-2xl font-bold text-foreground">{isEditing ? 'Edit Rack' : 'Add Rack'}</h2>
                <p className="text-sm text-muted-foreground font-medium">Configure server rack details and room location</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-bold text-muted-foreground uppercase">Rack Name *</label>
                <input 
                  type="text" 
                  value={formData.name} 
                  onChange={e => setFormData({...formData, name: e.target.value})} 
                  placeholder="e.g. Rack A1, Networking Rack..."
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-muted-foreground uppercase">Parent Room</label>
            ...
            <div className="px-8 py-6 border-t border-[#A3B1C6]/20 bg-background flex justify-end gap-4">
              ...
              <Button className="h-12 px-8 rounded-2xl border border-accent/50 font-bold text-white bg-accent shadow-[inset_2px_2px_4px_rgba(255,255,255,0.4),inset_-2px_-2px_4px_rgba(0,0,0,0.2),0_4px_10px_rgba(108,99,255,0.3)] hover:bg-accent-light transition-all" onClick={handleSave}>
                {isEditing ? 'Save Changes' : 'Create Rack'}
              </Button>
            </div>
          </div>
        </div>
