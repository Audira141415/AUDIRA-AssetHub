...
import { Box, Search, Plus, Edit2, Trash2, X, Server } from "lucide-react"
  ...
  return (
    <div className="relative min-h-[calc(100vh-80px)] overflow-hidden flex">
      <div className="flex-1 p-6 md:p-8 overflow-y-auto">
        <HeroSection title="Rooms" description="Manage server rooms" imageSrc="/images/heroes/rooms.png">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          <Card className="rounded-[24px] border-neu shadow-neu-extruded bg-background">
            <CardContent className="p-5 flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-500"><Box className="w-6 h-6" /></div>
              <div><p className="text-[10px] font-bold text-muted-foreground uppercase">Total Rooms</p><p className="text-2xl font-bold">{locations.length}</p></div>
            </CardContent>
          </Card>
          <Card className="rounded-[24px] border-neu shadow-neu-extruded bg-background">
            <CardContent className="p-5 flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-[#F59E0B]/10 flex items-center justify-center text-[#F59E0B]"><Server className="w-6 h-6" /></div>
        ...
        <Card className="rounded-[32px] border-neu shadow-neu-extruded bg-background overflow-hidden">
          <div className="overflow-x-auto pb-4">
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead>
                <tr>
                  <th className="px-6 py-6 font-bold text-xs uppercase text-muted-foreground border-b border-[#A3B1C6]/20">Room Name</th>
                  <th className="px-6 py-6 font-bold text-xs uppercase text-muted-foreground border-b border-[#A3B1C6]/20">Parent Floor</th>
              ...
              <tbody className="divide-y divide-[#A3B1C6]/10">
                {filtered.map(l => (
                  <tr key={l.id} className="hover:bg-[#A3B1C6]/10 transition-all">
                    ...
                    <td className="px-6 py-5">{floors.find(s => s.id === l.parentLoc)?.name || "-"}</td>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-xl h-full bg-background shadow-[-10px_0_30px_rgba(0,0,0,0.1)] flex flex-col">
            <div className="px-8 py-6 flex justify-between items-center border-b border-[#A3B1C6]/20">
              <h2 className="text-2xl font-bold">{isEditing ? 'Edit Room' : 'Add Room'}</h2>
              <Button variant="ghost" size="icon" onClick={() => setIsDrawerOpen(false)}><X className="w-5 h-5" /></Button>
            </div>
            <div className="flex-1 px-8 py-6 space-y-6">
              <div>
                <label className="text-xs font-bold text-muted-foreground">Room Name</label>
              ...
              <div>
                <label className="text-xs font-bold text-muted-foreground">Parent Floor</label>
