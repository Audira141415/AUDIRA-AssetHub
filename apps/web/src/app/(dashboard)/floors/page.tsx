...
import { mockFloors, mockRacks, baseAssets } from "@/lib/mock-data"
...
export default function FloorsPage() {
  const initialFloors: Floor[] = mockFloors;
  ...
  const [typeFilter, setTypeFilter] = useState("All Usage Type")
  const [sortConfig, setSortConfig] = useState<{key: keyof Floor, direction: 'asc'|'desc'} | null>(null)
  ...
  const filteredAndSortedFloors = useMemo(() => {
    ...
    if (typeFilter !== "All Usage Type") result = result.filter(r => r.type === typeFilter)
    ...
    return result
  }, [floors, searchQuery, statusFilter, typeFilter, sortConfig])
  ...
  const getSortIcon = (key: keyof Floor) => {
  ...
  const getBadgeColor = (type: string, value: string) => {
    ...
    if (type === "type") {
      if (value === "Data Center") return "text-blue-500 shadow-neu-inset-small bg-background"
      if (value === "Office") return "text-purple-500 shadow-neu-inset-small bg-background"
      if (value === "Mechanical") return "text-orange-500 shadow-neu-inset-small bg-background"
      return "text-gray-500 shadow-neu-inset-small bg-background"
    }
    return "text-gray-500 shadow-neu-inset-small bg-background"
  }

  const activeFloorData = floors.find(r => r.name === selectedFloor)

  // Dynamic Options for Selects
  const uniqueStatuses = ["All Status", ...Array.from(new Set(floors.map(r => r.status)))]
  const uniqueTypes = ["All Usage Type", ...Array.from(new Set(floors.map(r => r.type)))]

  return (
    <div className="flex w-full gap-6 pb-6">
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${selectedFloor ? 'pr-0' : ''}`}>
        ...
        <div className="flex flex-col xl:flex-row xl:items-start justify-between mb-6 gap-4">
          <div>
            <div className="flex flex-wrap items-center text-xs text-muted-foreground mb-2 font-medium">
              ...
              <ChevronRight size={12} className="mx-1" />
              <span className="text-foreground font-bold">Floors</span>
            </div>
            <h1 className="text-3xl font-bold text-foreground tracking-tight mb-1">Floors</h1>
            <p className="text-sm text-muted-foreground font-medium">Manage floors in Building A</p>
          </div>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(130px,1fr))] gap-4 mb-6">
          <div className="bg-background shadow-neu-extruded border-neu rounded-[20px] p-4 flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 overflow-hidden">
              <p className="text-[10px] font-bold text-muted-foreground mb-0.5 uppercase tracking-wider truncate">Total Floors</p>
              <div className="flex items-baseline gap-1 truncate">
                <span className="text-2xl font-black text-foreground">{floors.length}</span>
              </div>
              <p className="text-[9px] font-medium text-muted-foreground mt-0.5 truncate">All floors in building</p>
            </div>
          </div>
          <div className="bg-background shadow-neu-extruded border-neu rounded-[20px] p-4 flex items-center gap-3 min-w-0">
            <div className="w-12 h-12 rounded-2xl bg-background shadow-neu-inset-small flex items-center justify-center shrink-0">
            ...
            <div className="min-w-0 overflow-hidden">
              <p className="text-[10px] font-bold text-muted-foreground mb-0.5 uppercase tracking-wider truncate">Active Floors</p>
              <div className="flex items-baseline gap-1 truncate">
                <span className="text-2xl font-black text-foreground">{floors.filter(r => r.status === 'Active').length}</span>
              </div>
          ...
          <div className="bg-background shadow-neu-extruded border-neu rounded-[20px] p-4 flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 overflow-hidden">
              ...
              <div className="flex items-baseline gap-1 truncate">
                <span className="text-2xl font-black text-foreground">{floors.reduce((acc, curr) => acc + curr.assets, 0)}</span>
              </div>
              <p className="text-[9px] font-medium text-muted-foreground mt-0.5 truncate">Across all floors</p>
            </div>
          </div>
          <div className="bg-background shadow-neu-extruded border-neu rounded-[20px] p-4 flex items-center gap-3 min-w-0">
            <div className="w-12 h-12 rounded-2xl bg-background shadow-neu-inset-small flex items-center justify-center shrink-0">
            ...
            <div className="min-w-0 overflow-hidden">
              ...
              <div className="flex items-baseline gap-1 truncate">
                <span className="text-2xl font-black text-foreground">{floors.reduce((acc, curr) => acc + curr.racks, 0)}</span>
              </div>
          ...
          <div className="bg-background shadow-neu-extruded border-neu rounded-[20px] p-4 flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 overflow-hidden">
              ...
              <div className="flex items-baseline gap-1 truncate">
                <span className="text-2xl font-black text-foreground">{floors.reduce((acc, curr) => acc + curr.area, 0).toLocaleString()}</span>
                <span className="text-[10px] font-bold text-muted-foreground">m²</span>
              </div>
              <p className="text-[9px] font-medium text-muted-foreground mt-0.5 truncate">Total floor area</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col">
          {filteredAndSortedFloors.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center bg-background shadow-neu-extruded border-neu rounded-[32px] mb-2 p-12 text-center">
              ...
              <h3 className="text-2xl font-bold text-foreground mb-2">No floors found</h3>
              <p className="text-muted-foreground max-w-sm">We couldn't find any floors matching your current filters. Try adjusting your search query or clear the filters.</p>
              <Button 
                onClick={() => {
                  setSearchQuery("")
                  setStatusFilter("All Status")
            ...
            <div className="flex flex-col gap-4 mb-2">
                  ...
                  <table className="w-full text-sm text-left min-w-[900px]">
                    <thead className="text-xs text-muted-foreground uppercase bg-background shadow-neu-inset-small sticky top-0 z-10">
                      <tr>
                        ...
                        <th className="px-5 py-4 font-bold whitespace-nowrap group" onClick={() => handleSort('rooms')}>
                          <div className="flex items-center justify-end gap-1 cursor-pointer hover:text-foreground tracking-wider">
                    ...
                    <tbody>
                      {filteredAndSortedFloors.map((floor) => (
                        <tr 
                          key={floor.id} 
                          className={`border-b border-[#A3B1C6]/20 cursor-pointer transition-all duration-300 ${selectedFloor === floor.name ? 'shadow-neu-inset-small bg-background text-accent' : 'hover:shadow-neu-inset-small'}`}
                        ...
                        >
                          <td className="px-5 py-4 flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-background shadow-neu-extruded overflow-hidden p-0.5 shrink-0">
                              <img src={`https://images.unsplash.com/photo-${floor.id === '1' ? '1581444983059-4b68db601fbf' : '1504917595217-d4fb5054e20a'}?w=100&h=100&fit=crop`} alt={floor.name} className="w-full h-full object-cover rounded-lg mix-blend-overlay opacity-80" />
                            </div>
                            <div className="min-w-0">
                              <p className={`font-bold text-sm whitespace-nowrap truncate ${selectedFloor === floor.name ? 'text-accent' : 'text-foreground'}`}>{floor.name}</p>
                              <p className="text-[11px] text-muted-foreground font-medium mt-0.5 whitespace-nowrap truncate">{floor.desc}</p>
                            </div>
                          </td>
                          <td className="px-5 py-4 font-bold text-foreground whitespace-nowrap">{floor.code}</td>
                          <td className="px-5 py-4 whitespace-nowrap">
                            <span className={`px-2.5 py-1 text-[10px] rounded-lg font-bold ${getBadgeColor('type', floor.type)}`}>
                              {floor.type}
                            </span>
                          </td>
                          <td className="px-5 py-4 font-bold text-foreground text-right whitespace-nowrap">{floor.area.toLocaleString()}</td>
                          <td className="px-5 py-4 font-bold text-foreground text-right whitespace-nowrap">{floor.rooms}</td>
                          <td className="px-5 py-4 font-bold text-foreground text-right whitespace-nowrap">{floor.racks}</td>
                          <td className="px-5 py-4 font-bold text-foreground text-right whitespace-nowrap">{floor.assets}</td>
                          <td className="px-5 py-4 whitespace-nowrap">
                            <span className={`px-2.5 py-1 text-[10px] rounded-lg font-bold ${getBadgeColor('status', floor.status)}`}>
                              {floor.status}
                            </span>
                          </td>
                          <td className="px-5 py-4 text-right whitespace-nowrap relative">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <div className="relative" ref={openMenuId === floor.id ? menuRef : null}>
                                <Button 
                                  variant="ghost" 
                                  size="icon" 
                                  onClick={(e) => {
                                  ...
                                  className={`h-8 w-8 rounded-lg shadow-neu-extruded transition-all ${openMenuId === floor.id ? 'text-accent shadow-neu-inset-small' : 'text-muted-foreground hover:text-accent hover:shadow-neu-hover active:shadow-neu-inset-small'}`}
                                >
              ...
              <div className="bg-background shadow-neu-extruded rounded-[24px] p-4 flex flex-wrap gap-4 items-center justify-between text-sm text-muted-foreground font-medium shrink-0">
                <div>Showing 1 to {filteredAndSortedFloors.length} of {floors.length} floors</div>
            ...
            <div className="flex-1 overflow-y-auto mb-2 pr-2">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-6">
                {filteredAndSortedFloors.map(floor => (
                  <div 
                    key={floor.id}
                    className={`bg-background shadow-neu-extruded border-neu rounded-[32px] overflow-hidden flex flex-col cursor-pointer transition-all hover:shadow-neu-hover ${selectedFloor === floor.name ? 'ring-2 ring-accent ring-offset-4 ring-offset-background' : ''}`}
                    onClick={() => setSelectedFloor(floor.name)}
                  >
                    <div className="h-40 w-full relative p-2">
                      <div className="w-full h-full rounded-[24px] overflow-hidden shadow-neu-inset-deep">
                        <img src={`https://images.unsplash.com/photo-${floor.id === '1' ? '1581444983059-4b68db601fbf' : '1504917595217-d4fb5054e20a'}?w=600&h=300&fit=crop`} alt={floor.name} className="w-full h-full object-cover mix-blend-overlay opacity-80" />
                      </div>
                      <span className={`absolute top-5 right-5 px-3 py-1 text-[10px] rounded-lg font-bold shadow-neu-extruded bg-background ${floor.status === 'Active' ? 'text-green-500' : 'text-amber-500'}`}>
                        {floor.status}
                      </span>
                    </div>
                    <div className="p-6 flex-1 flex flex-col">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h3 className="text-xl font-bold text-foreground truncate">{floor.name}</h3>
                          <p className="text-sm text-muted-foreground font-medium truncate">{floor.code}</p>
                        </div>
                        <div className="relative" ref={openMenuId === floor.id ? menuRef : null}>
                          <Button 
                            variant="ghost" 
                          ...
                          {openMenuId === floor.id && (
                            <div className="absolute right-0 top-10 w-40 bg-background shadow-neu-extruded border-neu rounded-2xl z-50 py-2 animate-in fade-in zoom-in-95 duration-200">
                              ...
                              <div className="h-px w-full bg-[#A3B1C6]/20 my-1"></div>
                              <button className="w-full text-left px-4 py-2 text-sm font-bold text-red-500 hover:bg-background/50 hover:shadow-neu-inset-small transition-all">Delete Floor</button>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 mb-6 flex-wrap">
                        <span className={`px-2.5 py-1 text-[10px] rounded-lg font-bold ${getBadgeColor('type', floor.type)}`}>{floor.type}</span>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-4 mb-6 mt-auto">
                        <div className="bg-background shadow-neu-inset-small rounded-2xl p-3 text-center">
                          ...
                          <p className="text-xl font-black text-foreground">{floor.rooms}</p>
                        </div>
                        <div className="bg-background shadow-neu-inset-small rounded-2xl p-3 text-center">
                          <p className="text-xs font-bold text-muted-foreground mb-1">Racks</p>
                          <p className="text-xl font-black text-foreground">{floor.racks}</p>
                        </div>
                        <div className="bg-background shadow-neu-inset-small rounded-2xl p-3 text-center">
                          <p className="text-xs font-bold text-muted-foreground mb-1">Assets</p>
                          <p className="text-xl font-black text-foreground">{floor.assets}</p>
                        </div>
                      </div>
        ...
        <div className="w-[340px] bg-background shadow-neu-extruded border-neu rounded-[32px] flex flex-col h-full overflow-hidden shrink-0 transition-all mb-2 animate-in slide-in-from-right-8 duration-300">
          <div className="relative h-48 w-full p-2 shrink-0">
            <div className="w-full h-full rounded-[24px] overflow-hidden shadow-neu-inset-deep relative">
              <img src={`https://images.unsplash.com/photo-${activeFloorData.id === '1' ? '1581444983059-4b68db601fbf' : '1504917595217-d4fb5054e20a'}?w=600&h=400&fit=crop`} alt={activeFloorData.name} className="w-full h-full object-cover mix-blend-overlay opacity-80" />
            </div>
            <div className="absolute top-4 right-4 flex gap-2">
              <span className={`px-2.5 py-1 text-[10px] rounded-lg font-bold shadow-neu-extruded bg-background ${activeFloorData.status === 'Active' ? 'text-green-500' : 'text-amber-500'}`}>
                {activeFloorData.status}
              </span>
              <Button variant="ghost" size="icon" className="h-7 w-7 rounded-full shadow-neu-extruded bg-background text-foreground hover:text-red-500 hover:shadow-neu-hover active:shadow-neu-inset-small" onClick={() => setSelectedFloor(null)}>
                <X size={14} />
              </Button>
            </div>
            <div className="absolute bottom-4 left-5 right-5">
              <h2 className="text-xl font-black text-foreground mb-0.5 drop-shadow-md truncate">{activeFloorData.name}</h2>
              <p className="text-xs text-foreground font-bold drop-shadow-md truncate">{activeFloorData.desc} ({activeFloorData.building})</p>
            </div>
          </div>

          <div className="flex border-b border-[#A3B1C6]/20 px-2 pt-2 overflow-x-auto [&::-webkit-scrollbar]:hidden shrink-0">
            {['Overview', 'Rooms', 'Racks', 'Assets', 'History'].map(tab => (
              <button 
                key={tab}
          ...
          <div className="flex-1 overflow-y-auto p-5 relative">
            {activeTab === 'Overview' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className="grid grid-cols-[130px_1fr] gap-y-4 text-xs">
                  <div className="text-muted-foreground font-bold flex items-center gap-2"><Layout size={14} /> Floor Code</div>
                  <div className="text-foreground font-bold truncate">{activeFloorData.code}</div>
                  
                  <div className="text-muted-foreground font-bold flex items-center gap-2"><Box size={14} /> Building</div>
                  <div className="text-foreground font-bold truncate">{activeFloorData.building}</div>
                  
                  <div className="text-muted-foreground font-bold flex items-center gap-2"><MapPin size={14} /> Site</div>
                  <div className="text-foreground font-bold truncate">{activeFloorData.site}</div>
                  
                  <div className="text-muted-foreground font-bold flex items-center gap-2"><Layers size={14} /> Usage Type</div>
                  <div className="text-foreground font-bold truncate">{activeFloorData.type}</div>
                  
                  <div className="text-muted-foreground font-bold flex items-center gap-2"><Maximize2 size={14} /> Area</div>
                  <div className="text-foreground font-bold truncate">{activeFloorData.area.toLocaleString()} m²</div>
                  
                  <div className="text-muted-foreground font-bold flex items-center gap-2"><ArrowUp size={14} /> Raised Flr Height</div>
                  <div className="text-foreground font-bold truncate">{activeFloorData.raisedFloor}</div>
                  
                  <div className="text-muted-foreground font-bold flex items-center gap-2"><ArrowUp size={14} /> Floor Height</div>
                  <div className="text-foreground font-bold truncate">{activeFloorData.floorHeight}</div>
                  
                  <div className="text-muted-foreground font-bold flex items-center gap-2"><CheckCircle2 size={14} /> Status</div>
                  <div><span className={`px-2 py-0.5 text-[9px] rounded-lg ${getBadgeColor('status', activeFloorData.status)} font-bold`}>{activeFloorData.status}</span></div>
                  
                  <div className="text-muted-foreground font-bold flex items-start gap-2 pt-1"><Layout size={14} className="mt-0.5" /> Desc</div>
                  <div className="text-foreground font-medium text-[10px] leading-relaxed">Primary data center floor with high-density racks and infrastructure.</div>
                </div>

                <div className="pt-6 border-t border-[#A3B1C6]/20">
                  <h3 className="font-bold text-foreground text-sm tracking-tight mb-4">Floor Summary</h3>
                  
                  <div className="grid grid-cols-[130px_1fr] gap-y-3 text-xs mb-6">
                    <div className="text-muted-foreground font-bold flex items-center gap-2"><Layout size={14} /> Rooms</div>
                    <div className="text-foreground font-black text-right">{activeFloorData.rooms}</div>
                    
                    <div className="text-muted-foreground font-bold flex items-center gap-2"><Server size={14} /> Racks</div>
                    <div className="text-foreground font-black text-right">{activeFloorData.racks}</div>
                    
                    <div className="text-muted-foreground font-bold flex items-center gap-2"><Box size={14} /> Assets</div>
                    <div className="text-foreground font-black text-right">{activeFloorData.assets}</div>
                    
                    <div className="text-muted-foreground font-bold flex items-center gap-2"><Zap size={14} /> Power Capacity</div>
                    <div className="text-foreground font-black text-right">{activeFloorData.power.toLocaleString()} kW</div>
                    
                    <div className="text-muted-foreground font-bold flex items-center gap-2"><Zap size={14} /> Cooling Capacity</div>
                    <div className="text-foreground font-black text-right">{activeFloorData.cooling.toLocaleString()} kW</div>
                  </div>

                  <div className="space-y-2">
                    <div className="flex justify-between text-xs mb-1.5">
                      <span className="text-muted-foreground font-bold">Utilization</span>
                      <span className="text-foreground font-black">{activeFloorData.utilization}%</span>
                    </div>
                    <div className="h-2 w-full bg-background shadow-neu-inset-deep rounded-full overflow-hidden p-0.5">
                      <div className="h-full bg-accent rounded-full shadow-neu-extruded" style={{ width: `${activeFloorData.utilization}%` }}></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {['Rooms', 'History'].includes(activeTab) && (
              <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300">
                ...
                <p className="text-xs text-muted-foreground mb-6">View and manage all {activeTab.toLowerCase()} associated with {activeFloorData.name}.</p>
