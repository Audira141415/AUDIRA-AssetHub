...
import { 
  Building2, Box, Search, Plus, Edit2, Trash2, X, Globe, MapPin,
  TrendingUp, Clock, ShieldCheck, AlertTriangle, Layers, FileSpreadsheet, Zap
} from "lucide-react"
...
export default function BuildingsPage() {
  ...
  // Detail Drawer state
  const [selectedBuildingDetail, setSelectedBuildingDetail] = useState<any | null>(null)
  const [isExporting, setIsExporting] = useState(false)
  
  // Form states
  const [formData, setFormData] = useState({ name: "", type: "Building", parentLoc: "" })
  ...
  const buildings = useMemo(() => {
    return locations.filter((loc: any) => loc.type === "Building");
  }, [locations]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedBuildingDetail?.id === editingId) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (selectedBuildingDetail?.id === id) {
        setSelectedBuildingDetail(null);
      }
      fetchData();
    } catch (err: any) {
  ...
  const openCreateDrawer = () => {
    setFormData({ name: "", type: "Building", parentLoc: "" });
    setIsEditing(false);
    setEditingId(null);
    setIsDrawerOpen(true);
  }

  const openEditDrawer = (building: any, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    setFormData({ name: building.name, type: building.type, parentLoc: building.parentLoc || "" });
    setIsEditing(true);
  ...
  const totalAssets = buildings.reduce((sum, b) => sum + (b.assetsCount || 0), 0)
  const totalValuation = buildings.reduce((sum, b) => sum + (b.valuation || 0), 0)
  ...
  const handleExportCSV = async (building: any) => {
    try {
      setIsExporting(true);
      // Fetch all assets
      ...
      // Map location ID to parent building ID
      const locationMap = new Map();
      locations.forEach(loc => locationMap.set(loc.id, loc));

      const getParentBuildingId = (locationId: string | null): string | null => {
      ...
      // Filter assets by current building
      const buildingAssets = allAssets.filter((asset: any) => getParentBuildingId(asset.locationId) === building.id);

      // Create CSV content
      const headers = ["Asset Tag", "Hostname", "Category", "Status", "Lifecycle State", "Purchase Cost", "Purchase Date", "Last Audit"];
      const rows = buildingAssets.map((a: any) => [
        a.tag,
        a.hostname || "N/A",
        a.category?.name || "N/A",
        a.status || "Active",
      ...
      link.setAttribute("download", `audira_assets_building_${building.name.toLowerCase().replace(/[^a-z0-9]/g, '_')}.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } catch (err) {
      console.error("Export failed", err);
      alert("Failed to export building assets.");
    } finally {
    ...
    <div className="relative min-h-[calc(100vh-80px)] overflow-hidden flex bg-[#E0E5EC]">
      <div className="flex-1 transition-all duration-500 ease-in-out p-6 md:p-8 overflow-y-auto">
        <div className="space-y-8 pb-12 mt-2">
          ...
          <HeroSection
            title="Buildings"
            description="Manage data center physical buildings, facility hubs, and asset rollups."
            imageSrc="/images/heroes/buildings.png"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
            <Card className="rounded-[24px] border-neu shadow-neu-extruded bg-background">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-[#007DB8] uppercase tracking-wider mb-0.5 truncate">Total Buildings</p>
                  <p className="text-2xl font-display font-bold text-foreground truncate leading-none">{totalBuildings}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-[24px] border-neu shadow-neu-extruded bg-background">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-[#38B2AC] uppercase tracking-wider mb-0.5 truncate">Building Assets</p>
            ...
            <Card className="rounded-[24px] border-neu shadow-neu-extruded bg-background">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-[#F59E0B] uppercase tracking-wider mb-0.5 truncate">Total Valuation</p>
          ...
          {isLoading ? (
            <div className="text-center py-12 font-bold text-muted-foreground">Loading buildings...</div>
          ) : filteredBuildings.length === 0 ? (
            <div className="text-center py-12 font-bold text-muted-foreground">No buildings found.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredBuildings.map((building) => {
                const audit = getAuditStatus(building.lastAuditDate);
                const AuditIcon = audit.icon;
                const parentSiteName = sites.find(s => s.id === building.parentLoc)?.name || "Unknown Site";
                
                // Calculate percentages for status distribution
                const totalState = (building.statusBreakdown?.Active || 0) + 
                                   (building.statusBreakdown?.InStorage || 0) + 
                                   (building.statusBreakdown?.UnderRepair || 0) + 
                                   (building.statusBreakdown?.Retired || 0);

                const activePercent = totalState > 0 ? ((building.statusBreakdown?.Active || 0) / totalState) * 100 : 0;
                const storagePercent = totalState > 0 ? ((building.statusBreakdown?.InStorage || 0) / totalState) * 100 : 0;
                const repairPercent = totalState > 0 ? ((building.statusBreakdown?.UnderRepair || 0) / totalState) * 100 : 0;
                ...
                return (
                  ...
                  >
                    <div className="p-6 space-y-6">
                      {/* Top Row: Name & Parent Site */}
                      <div className="flex justify-between items-start gap-2">
                        <div>
                          <h3 className="font-display font-black text-xl text-foreground group-hover:text-accent transition-colors truncate max-w-[200px]" title={building.name}>
                            {building.name}
                          </h3>
                          <div className="flex items-center gap-1.5 mt-1 text-[11px] font-bold text-[#007DB8]">
                            <Globe size={12} />
                            <span>{parentSiteName}</span>
                          </div>
                          <div className="flex items-center gap-1 mt-1 text-[10px] font-bold text-muted-foreground">
                            <MapPin size={11} className="text-accent" />
                            <span>{building.floorsCount} Floor • {building.roomsCount} Room • {building.racksCount} Rack</span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
                          <button 
                            onClick={(e) => openEditDrawer(building, e)} 
                          ...
                          <button 
                            onClick={(e) => { e.stopPropagation(); handleDelete(building.id); }} 
                      ...
                      <div className="grid grid-cols-2 gap-2 bg-background shadow-neu-inset-small rounded-2xl p-4 border border-white/40">
                        <div>
                          ...
                          <p className="text-lg font-bold text-foreground font-display truncate">{formatCurrency(building.valuation || 0)}</p>
                        </div>
                        <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs font-bold text-muted-foreground">
                          <span>Assets: {building.assetsCount || 0} items</span>
                        ...
                        {totalState > 0 ? (
                          <div className="w-full h-2.5 bg-background shadow-neu-inset rounded-full overflow-hidden flex border border-white/20">
                            <div className="bg-accent h-full shadow-[0_0_8px_rgba(108,99,255,0.4)]" style={{ width: `${activePercent}%` }} title={`Active: ${building.statusBreakdown?.Active}`} />
                            <div className="bg-[#38B2AC] h-full" style={{ width: `${storagePercent}%` }} title={`In Storage: ${building.statusBreakdown?.InStorage}`} />
                            <div className="bg-[#F59E0B] h-full" style={{ width: `${repairPercent}%` }} title={`Under Repair: ${building.statusBreakdown?.UnderRepair}`} />
                          </div>
                        ) : (
                        ...
                        {totalState > 0 && (
                          <div className="flex flex-wrap gap-x-3 gap-y-1 text-[9px] font-bold text-muted-foreground mt-1">
                            <div className="flex items-center gap-1">
                              ...
                              <span>Active ({building.statusBreakdown?.Active || 0})</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <span className="w-1.5 h-1.5 rounded-full bg-[#38B2AC]" />
                              <span>Storage ({building.statusBreakdown?.InStorage || 0})</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <span className="w-1.5 h-1.5 rounded-full bg-[#F59E0B]" />
                              <span>Repair ({building.statusBreakdown?.UnderRepair || 0})</span>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="px-6 py-4 bg-background border-t border-[#A3B1C6]/10 flex items-center justify-between">
                      <span className="text-[10px] font-bold text-muted-foreground">
                        Last Audit: {building.lastAuditDate ? new Date(building.lastAuditDate).toLocaleDateString() : "Never"}
                      </span>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-xl h-full bg-background shadow-[-10px_0_30px_rgba(0,0,0,0.15)] border-l border-white/50 flex flex-col animate-in slide-in-from-right duration-300">
            <div className="px-8 py-6 flex justify-between items-center border-b border-[#A3B1C6]/20">
              <div>
                <h2 className="text-2xl font-bold text-foreground">{isEditing ? 'Edit Building' : 'Add Building'}</h2>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  placeholder="e.g. Batam Core Data Center"
                />
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-muted-foreground uppercase">Parent Site *</label>
                <select 
      ...
      {/* Detail Dashboard Drawer */}
      {selectedBuildingDetail && (
        <div className="fixed inset-0 z-40 flex justify-end">
          <div className="absolute inset-0 bg-black/25 backdrop-blur-sm" onClick={() => setSelectedBuildingDetail(null)}></div>
          ...
          <div className="relative w-full max-w-xl h-full bg-[#E0E5EC] shadow-[-10px_0_30px_rgba(0,0,0,0.15)] border-l border-white/60 flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="px-8 py-6 flex justify-between items-center border-b border-[#A3B1C6]/20 bg-background">
              <div>
                <span className="text-[10px] font-bold text-accent uppercase tracking-wider">Building Dashboard</span>
                <h2 className="text-2xl font-black text-foreground mt-0.5 leading-none">{selectedBuildingDetail.name}</h2>
              </div>
              <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl hover:bg-red-50 hover:text-red-500 hover:shadow-neu-inset" onClick={() => setSelectedBuildingDetail(null)}>
                <X className="w-5 h-5" />
              </Button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Card className="rounded-2xl border-neu shadow-neu-extruded bg-background p-4 flex flex-col justify-between">
                  ...
                  <p className="text-2xl font-display font-extrabold text-foreground mt-4">{formatCurrency(selectedBuildingDetail.valuation || 0)}</p>
                </Card>
                ...
                <Card className="rounded-2xl border-neu shadow-neu-extruded bg-background p-4 flex flex-col justify-between">
                  ...
                  <div className="mt-2 text-xs font-semibold text-foreground space-y-0.5">
                    <p>{selectedBuildingDetail.floorsCount || 0} Floors</p>
                    <p>{selectedBuildingDetail.roomsCount || 0} Rooms</p>
                    <p>{selectedBuildingDetail.racksCount || 0} Racks</p>
                  </div>
                </Card>
              </div>
                ...
                <div className="flex items-center justify-between p-3.5 bg-background shadow-neu-inset border-neu rounded-2xl">
                  <div>
                    ...
                    <p className="text-sm font-bold text-foreground mt-1">
                      {selectedBuildingDetail.lastAuditDate 
                        ? new Date(selectedBuildingDetail.lastAuditDate).toLocaleDateString(undefined, { dateStyle: 'long' }) 
                        : "Never Audited"
                      }
                    </p>
                  </div>
                  ...
                  {(() => {
                    const audit = getAuditStatus(selectedBuildingDetail.lastAuditDate);
                    const AuditIcon = audit.icon;
                    return (
              ...
              <Card className="rounded-[24px] border-neu shadow-neu-extruded bg-background p-5 space-y-4">
                ...
                <div className="space-y-3.5">
                  {[
                    { label: "Production / Active", count: selectedBuildingDetail.statusBreakdown?.Active || 0, color: "bg-accent" },
                    { label: "In Storage", count: selectedBuildingDetail.statusBreakdown?.InStorage || 0, color: "bg-[#38B2AC]" },
                    { label: "Under Repair / RMA", count: selectedBuildingDetail.statusBreakdown?.UnderRepair || 0, color: "bg-[#F59E0B]" },
                    { label: "Retired / Disposed", count: selectedBuildingDetail.statusBreakdown?.Retired || 0, color: "bg-red-500" },
                    { label: "Other", count: selectedBuildingDetail.statusBreakdown?.Other || 0, color: "bg-gray-400" }
                  ].map((state, idx) => {
                    const total = selectedBuildingDetail.assetsCount || 0;
              ...
              <Card className="rounded-[24px] border-neu shadow-neu-extruded bg-background p-5 space-y-4">
                ...
                <div className="divide-y divide-[#A3B1C6]/10 max-h-[250px] overflow-y-auto pr-1">
                  {Object.keys(selectedBuildingDetail.categoryDistribution || {}).length > 0 ? (
                    Object.entries(selectedBuildingDetail.categoryDistribution).map(([name, count]: any) => (
            ...
            <div className="px-8 py-6 border-t border-[#A3B1C6]/20 bg-background flex flex-col sm:flex-row justify-between gap-4">
              <Button 
                onClick={() => handleExportCSV(selectedBuildingDetail)}
                disabled={isExporting || (selectedBuildingDetail.assetsCount || 0) === 0} 
              ...
              >
                ...
                {isExporting ? "Exporting..." : "Export Building Inventory (CSV)"}
              </Button>

              <div className="flex gap-3 justify-end">
                <Button 
                  onClick={(e) => openEditDrawer(selectedBuildingDetail, e)} 
                ...
                <Button 
                  onClick={() => handleDelete(selectedBuildingDetail.id)} 
