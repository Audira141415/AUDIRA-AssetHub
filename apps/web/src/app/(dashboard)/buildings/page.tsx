...
export default function BuildingsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setLocations(res.data.filter((loc: any) => loc.type === "Building"));
  ...
  const totalBuildings = locations.length;
    ...
    <div className="relative min-h-[calc(100vh-80px)] overflow-hidden flex">
      <div className="flex-1 transition-all duration-500 ease-in-out p-6 md:p-8 overflow-y-auto">
        ...
        <div className="space-y-8 pb-12 mt-2">
          ...
          <HeroSection 
            title="Buildings" 
            description="Manage data center buildings and facilities" 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
            <Card className="rounded-[24px] border-neu shadow-neu-extruded bg-background">
              <CardContent className="p-5 flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-accent/10 flex items-center justify-center shadow-neu-inset text-accent">
                  <Building2 className="w-6 h-6" />
                </div>
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-accent uppercase tracking-wider mb-0.5 truncate">Total Buildings</p>
          ...
          <Card className="rounded-[32px] border-neu shadow-neu-extruded bg-background overflow-hidden">
            <div className="overflow-x-auto pb-4">
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr>
                    <th className="px-6 py-6 font-bold text-xs uppercase tracking-wider text-muted-foreground border-b border-[#A3B1C6]/20">Building Name</th>
                    <th className="px-6 py-6 font-bold text-xs uppercase tracking-wider text-muted-foreground border-b border-[#A3B1C6]/20">Parent Site</th>
                ...
                <tbody className="divide-y divide-[#A3B1C6]/10">
                  {isLoading ? (
                    <tr><td colSpan={4} className="px-6 py-12 text-center text-muted-foreground font-bold">Loading buildings...</td></tr>
                  ) : filtered.length === 0 ? (
                    <tr><td colSpan={4} className="px-6 py-12 text-center text-muted-foreground">No buildings found.</td></tr>
                  ) : (
                    filtered.map(l => (
                        ...
                        <td className="px-6 py-5">
                          {l.parentLoc ? (
                            <div className="flex items-center gap-2 text-muted-foreground font-medium">
                              <Globe className="w-4 h-4 text-[#007DB8]" />
                              <span>{sites.find(s => s.id === l.parentLoc)?.name || "Unknown Site"}</span>
                            </div>
                          ) : (
          ...
          <div className="relative w-full max-w-xl h-full bg-background shadow-[-10px_0_30px_rgba(0,0,0,0.1)] border-l border-white/50 flex flex-col animate-in slide-in-from-right duration-300">
            <div className="px-8 py-6 flex justify-between items-center border-b border-[#A3B1C6]/20">
              <div>
                ...
                <p className="text-sm text-muted-foreground font-medium">Configure building details and site hierarchy</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  placeholder="e.g. Building A, HQ Tower..."
